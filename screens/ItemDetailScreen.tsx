...
const ItemDetailScreen: React.FC<{ itemId: number }> = ({ itemId }) => {
  ...
  const openQuickAction = (type: 'checkout' | 'checkin') => {
      ...
      setQuickForm({
          jobId: defaultJobId,
          assignedToName: defaultAssignedName,
      ...
      try {
          if (quickAction === 'checkout') {
              ...
              await createTransaction({
                  ...
                  assignedToName: quickForm.assignedToName.trim(),
                  items: [{
                      itemId: item.id,
  ...
  // Get the person who has the item - prefer assignedToName (free text), fall back to looking up user by ID
  const currentPersonName = item.status === ItemStatus.CHECKED_OUT && item.history.length > 0
      ? (item.history[0].assignedToName || findUser(item.history[0].assignedToId || item.history[0].userId)?.name || 'Unknown')
      : null;
              ...
              <div className="bg-white dark:bg-slate-800 w-full max-w-md rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden animate-in zoom-in-95 duration-200">
                  ...
                  <div className="p-6 space-y-5">
                      ...
                      <div className="space-y-4">
                          ...
                          {quickAction === 'checkout' && (
                              <div>
                                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Who's Taking This?</label>
                                  <div className="relative">
                                      <User className="absolute left-3 top-3.5 text-slate-400" size={18}/>
                                      <input
                                          type="text"
                                          list="quick-user-suggestions"
                                          placeholder="Enter name (anyone)"
                                          ...
                                          value={quickForm.assignedToName}
                                          onChange={(e) => setQuickForm({ ...quickForm, assignedToName: e.target.value })}
                                      />
                                      <datalist id="quick-user-suggestions">
                                          {state.users.map(u => <option key={u.id} value={u.name} />)}
                                      </datalist>
                                  </div>
              ...
              <div className="bg-red-50 dark:bg-red-900/20 rounded-lg shadow-lg p-6 border-2 border-red-300 dark:border-red-700">
                  ...
                  {currentJob && (
                      <p className="text-sm text-slate-600 dark:text-slate-400">
                          For: <span onClick={() => navigateTo('JOB_DETAIL', {jobId: currentJob.id})} className="text-sky-600 dark:text-sky-400 hover:underline cursor-pointer font-medium">{currentJob.name}</span>
                      </p>
                  )}
              </div>
           )}
